...
SOURCE = 'fedlex.data.admin.ch (Fedlex SPARQL)'
...
_FAMILIES: list[dict] = [{'name': 'Federal legislation metadata', 'tool': 'ch_search_acts / ch_get_act'}]

_GAPS: list[dict] = [{'id': 'CH-001', 'family': 'Federal legislation metadata', 'missing': 'No full-text law content - this connector returns metadata (title, SR number), not the operative articles.', 'fallback': 'Follow source_url to Fedlex for the article text.'}, {'id': 'CH-002', 'family': 'Federal legislation metadata', 'missing': 'Cantonal law is out of scope - Fedlex publishes federal law.', 'fallback': "Use the relevant canton's legal collection."}, {'id': 'CH-003', 'family': 'Federal legislation metadata', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
