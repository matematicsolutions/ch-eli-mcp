...
from ch_eli_mcp.server import INSTRUCTIONS, mcp

SRC = (Path(__file__).parent.parent / "src" / "ch_eli_mcp" / "server.py").read_text(encoding="utf-8")
...
def test_instructions_only_reference_registered_tools():
    ...
    referenced = {r for r in _referenced_tool_names_in_instructions() if r in registered or r.startswith("ch_")}
