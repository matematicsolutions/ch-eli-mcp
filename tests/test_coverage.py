...
from ch_eli_mcp.coverage import build_coverage
from ch_eli_mcp.server import ch_coverage


def _coverage():
    fn = getattr(ch_coverage, "fn", ch_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CH_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("ch_coverage" in line for line in lines), (
        "an audit entry was written but it does not name ch_coverage"
    )
